...
"""Motion-tracking MONTAGE: N random motions stitched into ONE video, clean scene.

Renders the kinematic reference as marker dots with the SIMULATED robot playing on top,
in a bare environment (no table, no manipuland) via ``Isaac-Motion-Tracking-MotionOnly-v0``.

Each sampled motion is rendered for the full episode, then the env is reset onto the next
motion and frames keep appending to the SAME writer, so the output is a single continuous
video with a per-segment HUD label.

Modes (same wiring as play_sonic_adapter.py):
  --zero-residual        frozen SONIC only, no checkpoint needed (the baseline)
  --path <ckpt>          frozen SONIC + trained residual policy
"""
...
parser.add_argument("--motion-seed", type=int, default=0,
                    help="RNG seed for motion sampling (reproducible montages).")
parser.add_argument("--motion-ids", type=str, default=None,
                    help="Comma-separated explicit motion ids; overrides --num-motions/--motion-seed.")
parser.add_argument("--segment-steps", type=int, default=500,
                    help="Env steps rendered per motion (500 = 10 s at 50 Hz).")
...
def main():
    ...
    if args_cli.motion_ids:
        _mlist = [int(x) for x in args_cli.motion_ids.split(",") if x.strip() != ""]
    else:
        import random as _random
        _rng = _random.Random(args_cli.motion_seed)
        _k = min(int(args_cli.num_motions), _total)
        _mlist = sorted(_rng.sample(range(_total), _k))
    _seg = int(args_cli.segment_steps)
