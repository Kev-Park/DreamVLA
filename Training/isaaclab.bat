...
echo     -c, --conda [NAME]   Create the conda environment for Isaac Lab. Default name is 'dreamcontrol_51'.
echo.
goto :eof
...
if "%arg%"=="-i" (
    ...
    call :extract_python_exe
    rem install custom utility package if present
    if exist "%ISAACLAB_PATH%\isaac_utils\setup.py" (
        echo [INFO] Installing custom package: isaac_utils
        call !python_exe! -m pip install -e %ISAACLAB_PATH%\isaac_utils
    )
    ...
    if not "%~2"=="" (
        ...
        echo [INFO] Using default conda environment name: dreamcontrol_51
        set conda_env_name=dreamcontrol_51
    )
