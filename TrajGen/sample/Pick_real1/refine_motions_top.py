...
def compute_cost(joint_angles, trans, quats, ref_dists, offset_x=OFFSET_X, offset_z=0.88, debug=False):
    # L2 cost to target
    ...
    for link_name, tf in fk_results.items():
        if i == 36:
            ...
            cost2[0] += 2.*torch.norm(transformed_keypts[0] - transformed_keypts_ref[0], p=2)
            # import pdb; pdb.set_trace()
            # print(transformed_keypts_ref[0],transformed_keypts[0])
            transformed_keypts_ref[:, 0] += SHIFT_X
            transformed_keypts_ref[:, 1] += SHIFT_Y
            transformed_keypts_ref[:, 2] += SHIFT_Z

            transformed_keypts_ref[grab_idx-5:grab_idx, 2] = transformed_keypts_ref[grab_idx, 2]
            transformed_keypts_ref[grab_idx-5:grab_idx, 2] += SHIFT_Z_DOWN - SHIFT_Z_DOWN * torch.linspace(0., 1., 5).to(DEVICE)
            transformed_keypts_ref[grab_idx-5:grab_idx, 0] = transformed_keypts_ref[grab_idx, 0]
            transformed_keypts_ref[grab_idx-5:grab_idx, 1] = transformed_keypts_ref[grab_idx, 1]
            
            rel_dists = torch.norm(transformed_keypts[1:] - transformed_keypts[:-1], dim=1)
            ref_dists[:grab_idx] = torch.sum(ref_dists[:grab_idx]) / grab_idx
            ref_dists[60:] = 0.
            ...
            # import pdb; pdb.set_trace()
            cost2[grab_idx-5:] += torch.norm(transformed_keypts[grab_idx-5:] - transformed_keypts_ref[grab_idx-5:], dim=1, p=2)
            if debug :
                print(transformed_keypts[grab_idx:grab_idx+10])
            cost2 += 2*(transformed_keypts[:,0] + WRIST_TO_COLLISION > offset_x)*\
                    (transformed_keypts[:,2] < offset_z)*\
                    (torch.minimum(- offset_x + transformed_keypts[:,0] + WRIST_TO_COLLISION, - transformed_keypts[:,2] + offset_z))
        elif i == 38:
            rot_mat = tf.get_matrix()[:,:3,:3]
            rot_mat = torch.bmm(rot_matrix, rot_mat)
            rot_mat_ref = torch.tensor([[1, 0, 0], 
                                        [0, 0, 1], 
                                        [0, -1, 0]], dtype=torch.float32, device=DEVICE).T
...
for i in tqdm.tqdm(range(len(SHIFT_Xs))):
    ...
    grab_idx = motion_data["grab_idx"] - START_FROM
    ...
    for j in range(10000):
        ...
        if i > 5 and j >= 1500 :
            break
        if cost.item() < 0.017 and j > 150:
            cost = compute_cost(joint_angles, trans, quats, ref_dists, debug=True)
            break
...
width, height, depth = .5, 0.86, 2.0
cuboid = trimesh.creation.box(extents=(width, depth, height))
transform = np.eye(4)
transform[:3, 3] = [0.25+OFFSET_X, 0., 0.86/2.]
